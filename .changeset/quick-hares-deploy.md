...
One command deploys the site: `bunny sites deploy`.
...
- `bunny sites deploy` creates the storage zone, the Edge Script, and the pull zone on
...
- Each deploy lives in its own folder in the zone, and the CLI writes the folder's
  ...
  it was built with, and `bunny sites deployments publish` restores a page and its
  assets together.
...
**Breaking:** the `bunny-astro` command is gone. `bunny sites deploy` replaces
it, and adds deploys, rollback, and provisioning. A deploy you run yourself still works:
